...
//
//import com.greedystar.generator.config.Configuration;
...
//        mapperData.put("Configuration", configuration);
//        mapperData.put("ClassName", configuration.getName().getEntity()
...
//
//        mapperData.put("DaoClassName", configuration.getName().getDao()
//                .replace(Constant.PLACEHOLDER, invoker.getClassName()));
//        mapperData.put("TableName", invoker.getTableName());
//        mapperData.put("PrimaryKey", getPrimaryKeyColumnInfo(invoker.getColumnInfoList()).getColumnName());
//        mapperData.put("WhereId", "#{" + getPrimaryKeyColumnInfo(invoker.getColumnInfoList()).getPropertyName() + "}");
//        mapperData.put("PrimaryColumn", getPrimaryKeyColumnInfo(invoker.getColumnInfoList()));
...
//
//        String filePath;
//        if (configuration.isMapperUnderSource()) {
//            // mapper-under-source = true，表示将Mapper映射文件放在源文件目录下
//            filePath = FileUtil.getSourcePath() + StringUtil.package2Path(configuration.getPackageName())
//                    + StringUtil.package2Path(configuration.getPath().getMapper());
//        } else {
//            // 默认情况下，将Mapper映射文件放在resources下
//            filePath = FileUtil.getResourcePath() + StringUtil.package2Path(configuration.getPath().getMapper());
//        }
...
//        invoker.getColumnInfoList().forEach(ForEachUtil.withIndex((info, index) -> {
...
//        }));
//        if (invoker.getParentColumnInfoList() != null) {
//            invoker.getParentColumnInfoList().forEach(ForEachUtil.withIndex((info, index) -> {
...
//                if (info.getColumnName().equals(invoker.getForeignKey())) {
//                    sb.append(String.format("#{%s.%s},\n", StringUtil.firstToLowerCase(invoker.getParentClassName()),
//                            getPrimaryKeyColumnInfo(invoker.getParentColumnInfoList()).getPropertyName()));
//                } else {
...
//                    getPrimaryKeyColumnInfo(invoker.getColumnInfoList()).getColumnName()));
//            sb.append("\n").append(Constant.SPACE_8);
//            sb.append(String.format("LEFT JOIN `%s` ON `%s`.`%s` = `%s`.`%s`", invoker.getParentTableName(),
//                    invoker.getParentTableName(), getPrimaryKeyColumnInfo(invoker.getParentColumnInfoList()).getColumnName(),
